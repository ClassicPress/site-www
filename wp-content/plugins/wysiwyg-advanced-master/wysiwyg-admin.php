<?php
/**
 * This file is part of the WYSIWYG Advanced ClassicPress plugin and is released under the same license.
 * For more information please see wysiwyg-advanced.php.
 *
...
if ( isset( $_POST['tadv-save'] ) ) {
	...
	$message = '<div class="updated notice notice-success is-dismissible"><p>' .  __( 'Default settings restored.', 'wysiwyg-advanced' ) . '</p></div>';
	...
	?>
	<div class="wrap">
	<h2><?php _e( 'WYSIWYG Advanced Settings Export', 'wysiwyg-advanced' ); ?></h2>

	<div class="tadv-import-export">
	<p>
	<?php _e( 'The settings are exported as a JSON encoded string.', 'wysiwyg-advanced' ); ?>
	<?php _e( 'Please copy the content and save it in a <b>text</b> (.txt) file, using a plain text editor like Notepad.', 'wysiwyg-advanced' ); ?>
	<?php _e( 'It is important that the export is not changed in any way, no spaces, line breaks, etc.', 'wysiwyg-advanced' ); ?>
	</p>

	<form action="">
		<p><textarea readonly="readonly" id="tadv-export"><?php echo json_encode( $output ); ?></textarea></p>
		<p><button type="button" class="button" id="tadv-export-select"><?php _e( 'Select All', 'wysiwyg-advanced' ); ?></button></p>
	</form>
	<p><a href=""><?php _e( 'Back to WYSIWYG Settings', 'wysiwyg-advanced' ); ?></a></p>
	</div>
	</div>
	<?php
	...
	?>
	<div class="wrap">
	<h2><?php _e( 'WYSIWYG Advanced Settings Import', 'wysiwyg-advanced' ); ?></h2>

	<div class="tadv-import-export">
	<p><?php _e( 'The settings are imported from a JSON encoded string. Please paste the exported string in the text area below.', 'wysiwyg-advanced' );	?></p>

	<form action="" method="post">
		<p><textarea id="tadv-import" name="tadv-import"></textarea></p>
		<p>
			<button type="button" class="button" id="tadv-import-verify"><?php _e( 'Verify', 'wysiwyg-advanced' ); ?></button>
			<input type="submit" class="button button-primary alignright" name="tadv-import-submit" value="<?php _e( 'Import', 'wysiwyg-advanced' ); ?>" />
		</p>
	...
	if ( ! is_array( $import ) ) {
		$message = '<div class="error"><p>' .  __( 'Importing of settings failed.', 'wysiwyg-advanced' ) . '</p></div>';
	} else {
...
if ( empty( $this->toolbar_1 ) && empty( $this->toolbar_2 ) && empty( $this->toolbar_3 ) && empty( $this->toolbar_4 ) ) {
	$message = '<div class="error"><p>' . __( 'ERROR: All toolbars are empty. Default settings loaded.', 'wysiwyg-advanced' ) . '</p></div>';
...
<h2><?php _e( 'WYSIWYG Settings', 'wysiwyg-advanced' ); ?></h2>
<?php
...
if ( isset( $_POST['tadv-save'] ) && empty( $message ) ) {
	?><div class="updated notice notice-success is-dismissible"><p><?php _e( 'Settings saved.', 'wysiwyg-advanced' ); ?></p></div><?php
} else {
	echo $message;
}
...
<p class="tadv-submit">
	<input class="button-primary button-large" type="submit" name="tadv-save" value="<?php _e( 'Save Changes', 'wysiwyg-advanced' ); ?>" />
</p>
...
<?php _e( 'Enable the editor menu.', 'wysiwyg-advanced' ); ?>
</label></p>
...
<p><?php _e( 'Drag buttons from the unused buttons below and drop them in the toolbars above, or drag the buttons in the toolbars to rearrange them.', 'wysiwyg-advanced' ); ?></p>

<div id="unuseddiv">
<h3><?php _e( 'Unused Buttons', 'wysiwyg-advanced' ); ?></h3>
...
<div class="advanced-options">
	<h3><?php _e( 'Options', 'wysiwyg-advanced' ); ?></h3>
	<div>
		...
		<?php _e( 'List Style Options', 'wysiwyg-advanced' ); ?></label>
		<p>
			<?php _e( 'Enable more list options: upper or lower case letters for ordered lists, disk or square for unordered lists, etc.', 'wysiwyg-advanced' ); ?>
		</p>
	</div>
	<div>
		...
		<?php _e( 'Context Menu', 'wysiwyg-advanced' ); ?></label>
		<p>
			<?php _e( 'Replace the browser context (right-click) menu.', 'wysiwyg-advanced' ); ?>
		</p>
	</div>
	<div>
		...
		<?php _e( 'Alternative link dialog', 'wysiwyg-advanced' ); ?></label>
		<p>
			<?php _e( 'Open the TinyMCE link dialog when using the link button on the toolbar or the link menu item.', 'wysiwyg-advanced' ); ?>
		</p>
	</div>
	<div>
		...
		<?php _e( 'Font sizes', 'wysiwyg-advanced' ); ?></label>
		<p><?php printf( __( 'Replace the size setting available for fonts with: %s.', 'wysiwyg-advanced' ), $this->fontsize_formats ); ?></p>
	</div>
</div>
<?php
...
if ( ! is_multisite() || current_user_can( 'manage_sites' ) ) {
	?>
	<div class="advanced-options">
	<h3><?php _e( 'Advanced Options', 'wysiwyg-advanced' ); ?></h3>
	<?php
	...
	<div>
		...
		<?php _e( 'Create CSS classes menu', 'wysiwyg-advanced' ); ?></label>
		<p>
		<?php

		_e( 'Load the CSS classes used in editor-style.css and replace the Formats menu.', 'wysiwyg-advanced' );

		if ( $has_editor_style === 'not-supporetd' ) {
			?>
				<br>
				<span class="tadv-error"><?php _e( 'ERROR:', 'wysiwyg-advanced' ); ?></span>
		...
		if ( $has_editor_style === 'not-supporetd' || $disabled ) {
			_e( 'To use this option, add editor-style.css to your theme or a child theme. Enabling this option will also load that stylesheet in the editor.', 'wysiwyg-advanced' );
		}
	...
	<div>
		...
		<?php _e( 'Keep paragraph tags', 'wysiwyg-advanced' ); ?></label>
		<p>
			<?php _e( 'Stop removing the &lt;p&gt; and &lt;br /&gt; tags when saving and show them in the Text editor.', 'wysiwyg-advanced' ); ?>
			<?php _e( 'This will make it possible to use more advanced coding in the Text editor without the back-end filtering affecting it much.', 'wysiwyg-advanced' ); ?>
			<?php _e( 'However it may behave unexpectedly in rare cases, so test it thoroughly before enabling it permanently.', 'wysiwyg-advanced' ); ?>
			<?php _e( 'Line breaks in the Text editor would still affect the output, in particular do not use empty lines, line breaks inside HTML tags or multiple &lt;br /&gt; tags.', 'wysiwyg-advanced' ); ?>
		</p>
	</div>
	<div>
		...
		<?php _e( 'Enable pasting of image source', 'wysiwyg-advanced' ); ?></label>
		<p>
			<?php _e( 'Works only in Firefox and Safari. These browsers support pasting of images directly in the editor and convert them to base64 encoded text.', 'wysiwyg-advanced' ); ?>
			<?php _e( 'This is not acceptable for larger images like photos or graphics, but may be useful in some cases for very small images like icons, not larger than 2-3KB.', 'wysiwyg-advanced' ); ?>
			<?php _e( 'These images will not be available in the Media Library.', 'wysiwyg-advanced' ); ?>
		</p>
	</div>
	</div>

	<div class="advanced-options">
	<h3><?php _e( 'Administration', 'wysiwyg-advanced' ); ?></h3>
	<div>
		<h4><?php _e( 'Settings import and export', 'wysiwyg-advanced' ); ?></h4>
		<p>
			<input type="submit" class="button" name="tadv-export-settings" value="<?php _e( 'Export Settings', 'wysiwyg-advanced' ); ?>" /> &nbsp;
			<input type="submit" class="button" name="tadv-import-settings" value="<?php _e( 'Import Settings', 'wysiwyg-advanced' ); ?>" />
		</p>
	</div>
	<div>
		<h4><?php _e( 'Enable the editor enhancements for:', 'wysiwyg-advanced' ); ?></h4>
		<p>
			...
			<?php _e( 'The main editor (Add New and Edit posts and pages)', 'wysiwyg-advanced' ); ?></label>
		</p>
		<p>
			...
			<?php _e( 'Other editors in wp-admin', 'wysiwyg-advanced' ); ?></label>
		</p>
		<p>
			...
			<?php _e( 'Editors on the front end of the site', 'wysiwyg-advanced' ); ?></label>
		</p>
	</div>
	</div>
	<?php
...
<p class="tadv-submit">
	...
	<input class="button" type="submit" name="tadv-restore-defaults" value="<?php _e( 'Restore Default Settings', 'wysiwyg-advanced' ); ?>" />
	<input class="button-primary button-large" type="submit" name="tadv-save" value="<?php _e( 'Save Changes', 'wysiwyg-advanced' ); ?>" />
</p>
</form>

<div id="wp-adv-error-message" class="tadv-error">
<?php _e( 'The [Toolbar toggle] button shows or hides the second, third, and forth button rows. It will only work when it is in the first row and there are buttons in the second row.', 'wysiwyg-advanced' ); ?>
</div>
</div><!-- /wrap -->
